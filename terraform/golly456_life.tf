...
resource "aws_route53_record" "golly456_life_cloud_star_vii" {
  zone_id         = local.golly456_life_zone_id
  name            = "cloud.star.vii.golly456.life"
  type            = "A"
  allow_overwrite = true
  alias {
    name                   = "dhuidg3ekucji.cloudfront.net"
    zone_id                = "Z2FDTNDATAQYW2"
    evaluate_target_health = false
  }
}

resource "aws_route53_record" "golly456_life_caa_cloud_star_viii" {
  zone_id = local.golly456_life_zone_id
  name    = "cloud.star.viii.golly456.life"
  type    = "CAA"
  ttl     = 1799
  records = ["0 issue \"amazon.com\""]
}

resource "aws_route53_record" "golly456_life_acm_cloud_star_viii" {
  zone_id         = local.golly456_life_zone_id
  name            = "_a854d71330bd2d17d30df8859f491104.cloud.star.viii.golly456.life"
  type            = "CNAME"
  ttl             = 1799
...
resource "aws_api_gateway_domain_name" "golly456_life_apigw_cloud_star_viii" {
  provider        = aws.api_region
  domain_name     = "cloud.star.viii.golly456.life"
  certificate_arn = "arn:aws:acm:us-east-1:699103353083:certificate/c3bc0287-1e00-4aca-af00-aa9e95cf69a3"
}

resource "aws_api_gateway_base_path_mapping" "golly456_life_apigw_cloud_star_viii" {
  provider    = aws.api_region
  api_id      = "qynonhwyve"
  stage_name  = "integration"
  domain_name = aws_api_gateway_domain_name.golly456_life_apigw_cloud_star_viii.domain_name
}

resource "aws_route53_record" "golly456_life_cloud_star_viii" {
  zone_id         = local.golly456_life_zone_id
  name            = "cloud.star.viii.golly456.life"
  type            = "A"
  allow_overwrite = true
  alias {
    name                   = "dgoz6unv9ewai.cloudfront.net"
    zone_id                = "Z2FDTNDATAQYW2"
    evaluate_target_health = false
  }
}
...
resource "aws_route53_record" "golly456_life_cloud_viii" {
  zone_id         = local.golly456_life_zone_id
  name            = "cloud.viii.golly456.life"
  type            = "A"
  allow_overwrite = true
  alias {
    name                   = "d2xhza7lsfwxuy.cloudfront.net"
    zone_id                = "Z2FDTNDATAQYW2"
    evaluate_target_health = false
  }
}

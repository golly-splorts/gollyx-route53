...
resource "aws_route53_record" "golly_life_caa_cloud_viii" {
  zone_id = local.golly_life_zone_id
  name    = "cloud.viii.golly.life"
  type    = "CAA"
  ttl     = 1799
  records = ["0 issue \"amazon.com\""]
}

resource "aws_route53_record" "golly_life_acm_cloud_viii" {
  zone_id         = local.golly_life_zone_id
  name            = "_61b1338226b35ec8b4e219ad69bec855.cloud.viii.golly.life"
  type            = "CNAME"
  ttl             = 1799
  records         = ["_b0e5fb481f92d71a3abc158a354b87d9.jkddzztszm.acm-validations.aws"]
  allow_overwrite = true
}

resource "aws_api_gateway_domain_name" "golly_life_apigw_cloud_star_viii" {
  provider        = aws.api_region
  domain_name     = "cloud.star.viii.golly.life"
  certificate_arn = "arn:aws:acm:us-east-1:699103353083:certificate/3eb2e52d-8a0a-4efd-98c2-4b8336dc1483"
}

resource "aws_api_gateway_base_path_mapping" "golly_life_apigw_cloud_star_viii" {
  provider    = aws.api_region
  api_id      = "olbh6x7r21"
  stage_name  = "prod"
  domain_name = aws_api_gateway_domain_name.golly_life_apigw_cloud_star_viii.domain_name
}

resource "aws_route53_record" "golly_life_cloud_star_viii" {
  zone_id         = local.golly_life_zone_id
  name            = "cloud.star.viii.golly.life"
  type            = "A"
  allow_overwrite = true
  alias {
    name                   = "d1eyi8i8ia784g.cloudfront.net"
    zone_id                = "Z2FDTNDATAQYW2"
    evaluate_target_health = false
  }
}

resource "aws_api_gateway_domain_name" "golly_life_apigw_cloud_viii" {
  provider        = aws.api_region
  domain_name     = "cloud.viii.golly.life"
  certificate_arn = "arn:aws:acm:us-east-1:699103353083:certificate/f2301165-eebd-4b3d-acb4-cc54d4564e0e"
}

resource "aws_api_gateway_base_path_mapping" "golly_life_apigw_cloud_viii" {
  provider    = aws.api_region
  api_id      = "w7hgniqso5"
  stage_name  = "prod"
  domain_name = aws_api_gateway_domain_name.golly_life_apigw_cloud_viii.domain_name
}

resource "aws_route53_record" "golly_life_cloud_viii" {
  zone_id         = local.golly_life_zone_id
  name            = "cloud.viii.golly.life"
  type            = "A"
  allow_overwrite = true
  alias {
    name                   = "djgtzly89tqhd.cloudfront.net"
    zone_id                = "Z2FDTNDATAQYW2"
    evaluate_target_health = false
  }
}
